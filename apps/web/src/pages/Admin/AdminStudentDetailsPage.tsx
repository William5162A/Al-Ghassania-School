...
export function AdminStudentDetailsPage() {
  ...
  const [recordingCall] = useState(false);
  ...
  const profile = useMemo(() => {
    ...
    const student = students.find(
      (item) =>
        item.id === studentId && item.schoolId === schoolId
    );
    ...
    return {
      ...
      guardianName: guardian?.name ?? "غير محددة",
      guardianPhone: guardian?.phone ?? "غير محددة",
      violationRows,
  ...
  }, [studentId, schoolId, schoolName]);

  if (studentId && !students.some((item) => item.id === studentId)) {
  ...
  const handleRecordCall = () => {
    const nextCallNumber = getNextCallNumber(student.id, guardianCalls);
    recordGuardianCall(
      student.id,
      student.guardianId,
      schoolId,
    ...
    addSessionActivity({
      ...
      message: `استدعاء ولي أمر الطالب ${student.fullName} - الاتصال رقم ${nextCallNumber}`,
      studentId: student.id,
      studentName: student.fullName,
    });
    // Refresh will happen automatically via mockData update
  };
    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card-background)] p-5 shadow-[var(--shadow-sm)]">
          <p className="text-sm text-[var(--text-secondary)]">المدرسة</p>
        ...
        <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card-background)] p-5 shadow-[var(--shadow-sm)]">
          <p className="text-sm text-[var(--text-secondary)]">إجمالي النقاط</p>
        ...
        <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card-background)] p-5 shadow-[var(--shadow-sm)]">
          <p className="text-sm text-[var(--text-secondary)]">نقاط الدورة الحالية</p>
        ...
        <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card-background)] p-5 shadow-[var(--shadow-sm)]">
          <p className="text-sm text-[var(--text-secondary)]">عدد المخالفات</p>
        ...
        <div className="rounded-2xl border border-[var(--card-border)] bg-[var(--card-background)] p-5 shadow-[var(--shadow-sm)]">
          <p className="text-sm text-[var(--text-secondary)]">الشعبة</p>
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-3">
          ...
          <div className="sm:col-span-3">
            <button
              ...
              disabled={recordingCall}
              className="w-full rounded-xl bg-[var(--brand-primary)] px-5 py-2.5 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50"
            >
              {recordingCall ? "جاري التسجيل..." : "تسجيل استدعاء ولي أمر"}
            </button>
          </div>
        </div>
      </section>
