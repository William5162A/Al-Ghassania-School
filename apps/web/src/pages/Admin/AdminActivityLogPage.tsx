...
} from "../Owner/sessionActivity";
import type { SessionActivityType } from "../Owner/sessionActivity";
...
const TYPE_LABELS: Record<LogEntry["type"], string> = {
  ...
  guardian_call_required: "طلب اتصال بولي الأمر",
...
const TYPE_ICONS: Record<LogEntry["type"], string> = {
  ...
  violation_points_edited: "✏️",
  violation_point_deleted: "🗑️",
};
...
export function AdminActivityLogPage() {
  ...
  const schoolStudentIds = useMemo(
    () =>
      new Set(
        students
    ...
    guardianCalls
      ...
      .forEach((call) => {
        ...
        list.push({
          ...
          message: "تم استدعاء ولي الأمر بنجاح",
          studentId: student?.id,
          studentName: student?.fullName ?? "غير محددة",
          date: call.date,
          sourceLabel: "النظام",
    ...
    sessionActivities
      .filter((activity) => {
        if (activity.studentId) {
          return schoolStudentIds.has(activity.studentId);
        }
        return true;
      })
      .forEach((activity) => {
  ...
  }, [schoolId, sessionActivities, schoolStudentIds]);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
            سجل النشاطات والأحداث الخاصة بمدرستك فقط. الأحداث المسجلة خارج الجلسة الحالية تُعد سجلاً دائماً.
          </p>
        </div>
      ...
      <section className="rounded-2xl border border-[var(--card-border)] bg-[var(--card-background)] shadow-[var(--shadow-sm)]">
        <div className="border-b border-[var(--card-border)] p-5">
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">
            {filtered.length.toLocaleString("en-US")} عملية مطابقة حسب التصفية الحالية
          </p>
        </div>
            ...
            <table className="w-full min-w-0 text-right">
              <thead>
                <tr className="border-b border-[var(--border-light)] text-xs text-[var(--text-muted)]">
                  <th className="px-5 py-3 font-medium">نوع النشاط</th>
                  <th className="px-5 py-3 font-medium">الحدث المرتبط</th>
                          ...
                          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold ${
                            ...
                            entry.type === "violation_type_added"
                              ...
                              : entry.type === "guardian_call_required" ||
                                entry.type === "violation_point_deleted"
